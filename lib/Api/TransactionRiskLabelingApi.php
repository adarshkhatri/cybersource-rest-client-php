...
/**
 * TransactionRiskLabelingApi Class Doc Comment
 *
 * @category Class
 * @package  CyberSource
 * @author   Swagger Codegen team
...
class TransactionRiskLabelingApi
{
     ...
     *
     * @return TransactionRiskLabelingApi
     */
    ...
    /**
     * Operation submitLabels
     *
     * Transaction Risk Labeling
     *
     * @param \CyberSource\Model\LabelRequest $labelRequest Label submission request (required)
     * @throws \CyberSource\ApiException on non-2xx response
     * @return array of \CyberSource\Model\InlineResponse2013, HTTP status code, HTTP response headers (array of strings)
     */
    public function submitLabels($labelRequest)
    {
        self::$logger->info('CALL TO METHOD submitLabels STARTED');
        list($response, $statusCode, $httpHeader) = $this->submitLabelsWithHttpInfo($labelRequest);
        self::$logger->info('CALL TO METHOD submitLabels ENDED');
        self::$logger->close();
        return [$response, $statusCode, $httpHeader];
    }

    /**
     * Operation submitLabelsWithHttpInfo
     *
     * Transaction Risk Labeling
     *
     * @param \CyberSource\Model\LabelRequest $labelRequest Label submission request (required)
     ...
     * @return array of \CyberSource\Model\InlineResponse2013, HTTP status code, HTTP response headers (array of strings)
     */
    public function submitLabelsWithHttpInfo($labelRequest)
    {
        // verify the required parameter 'labelRequest' is set
        if ($labelRequest === null) {
            self::$logger->error("InvalidArgumentException : Missing the required parameter $labelRequest when calling submitLabels");
            throw new \InvalidArgumentException('Missing the required parameter $labelRequest when calling submitLabels');
        }
        // parse inputs
        $resourcePath = "/unifiedrisk";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        if (isset($labelRequest)) {
            $_tempBody = $labelRequest;
        }
        
        $sdkTracker = new \CyberSource\Utilities\Tracking\SdkTracker();
        $modelClassLocation = explode('\\', '\CyberSource\Model\LabelRequest');
        ...
        if (MLEUtility::checkIsMLEForAPI($this->apiClient->merchantConfig, $inboundMLEStatus, "submitLabels,submitLabelsWithHttpInfo")) {
            try {
        ...
        // Response MLE check
        $isResponseMLEForAPI = MLEUtility::checkIsResponseMLEForAPI($this->apiClient->merchantConfig, "submitLabels,submitLabelsWithHttpInfo");
        ...
        try {
            ...
            switch ($e->getCode()) {
                case 201:
                    ...
                    break;
                case 400:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\CyberSource\Model\UnifiedRiskLabelsPost400Response', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                case 500:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\CyberSource\Model\UnifiedRiskLabelsPost400Response', $e->getResponseHeaders());
